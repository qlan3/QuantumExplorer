...
class AtariEncoder(nn.Module):
  '''
  The VAE encoder for Atari games
  '''
  def __init__(self, in_channels=4, feature_dim=512):
    super().__init__()
    self.feature_dim = feature_dim
    self.CVnet = nn.Sequential(
    ...
    linear_input_size = 7 * 7 * 64
    self.mu = layer_init(nn.Linear(linear_input_size, feature_dim))
    self.log_std = layer_init(nn.Linear(linear_input_size, feature_dim))

  def forward(self, x):
    h = self.CVnet(x)
    h = h.view(h.size(0), -1)
    mu, log_std = self.mu(h), self.log_std(h)
    return mu, log_std


class AtariDecoder(nn.Module):
  '''
  The VAE dncoder for Atari games
  '''
  def __init__(self, out_channels, feature_dim=512):
    self.feature_dim = feature_dim
    linear_input_size = 7 * 7 * 64
    self.fc = layer_init(nn.Linear(feature_dim, linear_input_size))
    self.CVnet = nn.Sequential(
      layer_init(nn.ConvTranspose2d(64, 64, kernel_size=3, stride=1)),
      nn.ReLU(),
      layer_init(nn.ConvTranspose2d(64, 32, kernel_size=4, stride=2)),
      nn.ReLU(),
      layer_init(nn.ConvTranspose2d(32, out_channels, kernel_size=8, stride=4)),
      nn.Sigmoid()
    )
...
class VAE_MinAtar(nn.Module):
  '''
  A VAE network for MinAtar games
  '''
  def __init__(self, channels, feature_dim=128):
    ...
    self.encoder = nn.Sequential(
      layer_init(nn.Conv2d(in_channels=channels, out_channels=16, kernel_size=3, stride=1)),
      nn.ReLU()
    )
    linear_input_size = size_linear_unit(10) * size_linear_unit(10) * 16
    self.mu = layer_init(nn.Linear(linear_input_size, feature_dim))
    ...
    self.decoder = nn.Sequential(
      layer_init(nn.ConvTranspose2d(in_channels=16, out_channels=channels, kernel_size=3, stride=1)),
      nn.Sigmoid()
    )
...
class VAE_MinAtar2(nn.Module):
  '''
  A more complex VAE network for MinAtar games
  '''
  def __init__(self, channels, feature_dim=128):
    super().__init__()
    self.feature_dim = feature_dim
    ...
    self.encoder = nn.Sequential(
      ...
      nn.ReLU(),
      layer_init(nn.Conv2d(32, 32, kernel_size=1, stride=1)),
      nn.ReLU()
    )
    self.mu = layer_init(nn.Linear(linear_input_size, feature_dim))
    self.log_std = layer_init(nn.Linear(linear_input_size, feature_dim))
    ...
    self.fc = layer_init(nn.Linear(feature_dim, linear_input_size))
    self.decoder = nn.Sequential(
      layer_init(nn.ConvTranspose2d(32, 32, kernel_size=1, stride=1)),
      nn.ReLU(),
      layer_init(nn.ConvTranspose2d(32, 16, kernel_size=2, stride=1)),
      nn.ReLU(),
      layer_init(nn.ConvTranspose2d(16, out_channels, kernel_size=3, stride=1)),
      nn.Sigmoid()
    )
...
class VAE_DQNNet(nn.Module):
  def __init__(self, VAE_net, feature_net, value_net):
    super().__init__()
    # VAE net
    self.VAE = VAE_net
    self.vae_params = list(self.VAE.parameters())
    self.feature_dim = self.VAE.feature_dim
    # DQN net
    self.feature_net = feature_net
    self.value_net = value_net
    self.dqn_params = list(self.feature_net.parameters()) + list(self.value_net.parameters())

  def sample_feature(self, mu, log_std):
    # Apply the reparameterization technique
  ...
  def sample_data(self, batch_size, device):
    ...
    phi = torch.randn(batch_size, self.feature_dim, device=device)
    data = self.VAE.decode(phi)
    return data.clone().detach()

  def forward(self, obs, vae_flag=False):
    if vae_flag: # VAE
      # Sample the latent feature
      mu, log_std = self.VAE.encode(obs)
      phi = self.sample_feature(mu, log_std)
      recons_obs = self.VAE.decode(phi)
      recons_loss = nn.MSELoss(reduction='mean')(obs, recons_obs)
      log_var = 2.0*log_std
